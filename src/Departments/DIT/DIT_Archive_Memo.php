...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // First, check if the memo exists and is approved
    $checkQuery = "SELECT * FROM DIT_post WHERE id=? AND title='Memo' AND (status='approved' OR status='Approved')";
    ...
    if ($result->num_rows === 0) {
        echo json_encode(['success' => false, 'message' => 'Memo not found or not in approved status']);
        exit;
    }
