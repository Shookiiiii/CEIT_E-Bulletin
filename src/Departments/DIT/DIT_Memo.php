...
while ($row = $result->fetch_assoc()) {
 ...
 $query = "SELECT * FROM DIT_post WHERE title='memo' AND status='archived'";
 $result = $conn->query($query);
while ($row = $result->fetch_assoc()) {
    $archived[] = [
        'file_path' => 'uploads/' . $row['file_path'],
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-orange-600 mb-4 md:mb-0">DIT Memos</h1>
            <button onclick="openPdfModal()"
                ...
                <i class="fas fa-upload"></i> Upload Memo
            </button>
        </div>
                    ...
                    <?php foreach ($approved as $index => $pdf): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-green-500 transition duration-200 transform hover:scale-105">
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="view-full-approved-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="pdf" data-file-path="<?= $pdf['file_path'] ?>"> 
                                    <i class="fas fa-eye fa-sm"></i>
                                    View
                                </button>
                                <button class="p-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition duration-200 transform hover:scale-110 memo-delete-btn" data-index="<?= $index ?>" data-id="<?= $pdf['id'] ?>" data-description="<?= htmlspecialchars($pdf['description']) ?>" title="Delete">
                    ...
                    <?php foreach ($pending as $index => $pdf): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-yellow-500 transition duration-200 transform hover:scale-105">
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                ...
                                <button class="p-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition duration-200 transform hover:scale-110 edit-btn" data-index="<?= $index ?>" data-id="<?= $pdf['id'] ?>" title="Edit">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($not_approved as $index => $pdf): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-red-500 transition duration-200 transform hover:scale-105">
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs rounded-lg">
                                <button id="view-full-not-approved-<?= $index ?>" class="p-2 rounded-lg border border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="pdf" data-file-path="<?= $pdf['file_path'] ?>"> 
                                    <i class="fas fa-eye fa-sm"></i>
                                    View
                                </button>
                                <button class="p-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition duration-200 transform hover:scale-110 memo-delete-btn" data-index="<?= $index ?>" data-id="<?= $pdf['id'] ?>" data-description="<?= htmlspecialchars($pdf['description']) ?>" title="Delete">
    ...
    <div id="edit-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 class="text-xl font-bold mb-4">Edit Memo</h2>
            <form id="edit-form" class="space-y-4" enctype="multipart/form-data">
                ...
                <div>
                    <label for="edit-file" class="block text-sm font-medium text-gray-700">Replace PDF (optional)</label>
                    <input
                        type="file"
                        id="edit-file"
                        name="file"
    ...
    <div id="delete-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-2xl cursor-pointer text-gray-500 hover:text-red-600" onclick="closeDeleteModal()">&times;</span>
            </div>
            <div class="mb-4">
                <p>Would you like to archive or delete this memo?</p>
                <p class="font-semibold mt-2" id="delete-memo-title"></p>
            </div>
            <div class="flex justify-end space-x-3">
                <button class="border border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white px-4 py-2 rounded-md transition duration-200 transform hover:scale-110" onclick="closeDeleteModal()">
                    Cancel
                </button>
                <button class="border border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-white px-4 py-2 rounded-md transition duration-200 transform hover:scale-110" id="confirm-archive-btn">
                    <i class="fas fa-archive mr-2"></i> Archive
                </button>
                <button class="border border-red-500 text-red-500 hover:bg-red-500 hover:text-white px-4 py-2 rounded-md transition duration-200 transform hover:scale-110" id="confirm-delete-btn">
    ...
    <!-- PDF Upload Modal -->
    <div id="pdfUploadModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 class="text-xl font-bold mb-4">Upload Memo</h2>
            <form action="DIT_Add_Memo.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                <div>
                ...
                <div class="flex justify-end space-x-3">
                    <button
                        ...
                        class=" px-4 py-2 border border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white rounded-md transition duration-200 transform hover:scale-110">
                        Cancel
                    </button>
                    <button
                        type="submit"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderPdfPreviews(pdfs, status) {
                pdfs.forEach((pdf, index) => {
                    ...
                    document.getElementById(`view-full-${status}-${index}`).addEventListener("click", function() {
                        ...
                        pdfjsLib.getDocument(filePath).promise.then(pdfDoc => {
                            ...
                            document.getElementById(`pageIndicator-${status}-${index}`).textContent = `Page 1 of ${totalPages[`${status}-${index}`]}`;
                            
                            // Enable/disable navigation buttons
                            document.getElementById(`prevPageBtn-${status}-${index}`).disabled = true;
                            document.getElementById(`nextPageBtn-${status}-${index}`).disabled = totalPages[`${status}-${index}`] <= 1;
                    ...
                    document.getElementById(`prevPageBtn-${status}-${index}`).addEventListener('click', function() {
                        goToPrevPage(status, index);
                    });
                    
                    document.getElementById(`nextPageBtn-${status}-${index}`).addEventListener('click', function() {
                        goToNextPage(status, index);
                    });
                    
                    // Close modal when clicking outside content
                    document.getElementById(`file-modal-${status}-${index}`).addEventListener("click", function(e) {
                        if (e.target === this) {
                            closeFileModal(status, index);
                        }
                    });
                });
            }
            ...
            function goToPrevPage(status, index) {
                ...
                if (currentPageNum[key] > 1) {
                    ...
                    // Update navigation
                    document.getElementById(`prevPageBtn-${status}-${index}`).disabled = currentPageNum[key] === 1;
                    document.getElementById(`nextPageBtn-${status}-${index}`).disabled = false;
                    document.getElementById(`pageIndicator-${status}-${index}`).textContent = `Page ${currentPageNum[key]} of ${totalPages[key]}`;
                }
            }
            ...
            function goToNextPage(status, index) {
                ...
                if (currentPageNum[key] < totalPages[key]) {
                    ...
                    // Update navigation
                    document.getElementById(`prevPageBtn-${status}-${index}`).disabled = false;
                    document.getElementById(`nextPageBtn-${status}-${index}`).disabled = currentPageNum[key] === totalPages[key];
                    document.getElementById(`pageIndicator-${status}-${index}`).textContent = `Page ${currentPageNum[key]} of ${totalPages[key]}`;
                }
            }
            ...
            // Edit button functionality (only for pending memos)
            ...
            document.getElementById('edit-form').addEventListener('submit', function(e) {
                ...
                if (fileInput.files.length > 0) {
                    ...
                    // Format: memo_DD_MM_YYYY_HH_MM_SS.pdf
                    const newFilename = `memo_${day}_${month}_${year}_${hours}_${minutes}_${seconds}.pdf`;
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert('Error updating memo: ' + data.message);
                    }
                })
                .catch(error => {
...
let deleteId = null;
document.querySelectorAll('.memo-delete-btn').forEach(button => {
    button.addEventListener('click', function() {
        const id = this.getAttribute('data-id');
        ...
        // Set the memo title in the delete confirmation modal
        document.getElementById('delete-memo-title').textContent = description;
                ...
                if (deleteId) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert('Error archiving memo: ' + data.message);
                            // Re-enable the button
                            document.getElementById('confirm-archive-btn').disabled = false;
                            document.getElementById('confirm-archive-btn').innerHTML = '<i class="fas fa-archive mr-2"></i> Archive';
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        alert('An error occurred while archiving the memo');
                        // Re-enable the button
                        document.getElementById('confirm-archive-btn').disabled = false;
                        document.getElementById('confirm-archive-btn').innerHTML = '<i class="fas fa-archive mr-2"></i> Archive';
                    });
                ...
                if (deleteId) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert('Error deleting memo: ' + data.message);
                            // Re-enable the button
                            document.getElementById('confirm-delete-btn').disabled = false;
                            document.getElementById('confirm-delete-btn').innerHTML = '<i class="fas fa-trash mr-2"></i> Delete';
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
