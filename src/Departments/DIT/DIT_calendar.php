...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === "delete" && $id) {
        ...
        try {
            $result = $conn->query("DELETE FROM dit_calendar WHERE id = $id");
            if ($result) {
    ...
    if ($action === "save") {
        if ($id) {
            $conn->query("UPDATE dit_calendar SET start_date='$start_date', end_date='$end_date', description='$desc', semester=$semester WHERE id=$id");
            echo json_encode(["success" => true, "updated" => true]);
        } else {
            $conn->query("INSERT INTO dit_calendar (start_date, end_date, description, semester) VALUES ('$start_date', '$end_date', '$desc', $semester)");
...
function renderSemesterTable($conn, $semester)
{
    ...
    $query = "SELECT * FROM dit_calendar WHERE semester = $semester ORDER BY start_date ASC";
