...
<body class="bg-white text-gray-800">
    <div class="w-full">
        <nav class="flex justify-center space-x-10 border-b-2 border-gray-200 mb-6">
            ...
            <button class="dit-tab-btn text-gray-700 hover:border-b-2 hover:border-orange-400 hover:text-orange-400 hover:font-bold pb-2" data-subtab="dit-about">About DIT</button>
        </nav>

        <div id="dit-announcement" class="dit-subtab">
            <?php include '../DIT_ManageAnnouncements.php'; ?>
        </div>

        <div id="dit-memo" class="dit-subtab hidden">
            <?php include '../DIT_ManageMemo.php'; ?>
        </div>

        <div id="dit-graphs" class="dit-subtab hidden">
            <?php include 'DIT_graphs.php'; ?>
        </div>

        <div id="dit-about" class="dit-subtab hidden">
            <h1>About here</h1>
        </div>
    </div>

    <!-- Reject Modals -->
    <?php include 'DIT_reject_modals.php'; ?>

    <!-- File View Modals -->
    <?php include 'DIT_file_modals.php'; ?>
    ...
    `;
        ...
        `;
            ...
            setupGraphApproveRejectButtons: function() {
                const self = this;
                // Approve buttons for graphs
                document.querySelectorAll('.graph-approve-btn').forEach(button => {
                    button.addEventListener('click', function() {
                        const id = this.getAttribute('data-id');
                        self.approveGraph(id, this);
                    });
                });
                // Reject buttons for graphs
            ...
            submitGraphRejection: function(event) {
                ...
                fetch('../DIT/DIT_Approve_Reject_Graph.php', {
                        ...
                        body: `id=${this.currentGraphRejectId}&action=reject&reason=${encodeURIComponent(reason.value.trim())}`
                    })
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Network response was not ok');
            ...
            initializeCharts: function() {
                // Initialize charts for approved graphs
                <?php
                global $approved_graphs;
                if (isset($approved_graphs) && is_array($approved_graphs)):
                    foreach ($approved_graphs as $index => $graph): ?>
                        const ctxApproved<?= $index ?> = document.getElementById('chart-approved-<?= $index ?>');
                        new Chart(ctxApproved<?= $index ?>, {
                            type: 'doughnut',
                            data: {
                ...
                // Initialize charts for pending graphs
                <?php
                if (isset($pending_graphs) && is_array($pending_graphs)):
                    foreach ($pending_graphs as $index => $graph): ?>
                        const ctxPending<?= $index ?> = document.getElementById('chart-pending-<?= $index ?>');
                        new Chart(ctxPending<?= $index ?>, {
                            type: 'doughnut',
                            data: {
                                labels: <?= json_encode(array_column($graph['data'], 'label')) ?>,
                                datasets: [{
                ...
                // Initialize charts for not approved graphs
                <?php
                if (isset($not_approved_graphs) && is_array($not_approved_graphs)):
                    foreach ($not_approved_graphs as $index => $graph): ?>
                        const ctxNotApproved<?= $index ?> = document.getElementById('chart-not-approved-<?= $index ?>');
                        new Chart(ctxNotApproved<?= $index ?>, {
                            type: 'doughnut',
                            data: {
                                labels: <?= json_encode(array_column($graph['data'], 'label')) ?>,
                                datasets: [{
