...
// Function to load all graphs from the DIT_post table (for DIT carousel)
function loadDITGraphs($conn) {
    $graphs = [];
    $query = "SELECT * FROM DIT_post WHERE title='graph' AND status='approved' ORDER BY created_at ASC";
    $result = $conn->query($query);
    
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $graphData = json_decode($row['content'], true);
            if ($graphData && isset($graphData['title']) && isset($graphData['data'])) {
...
?>
<!-- graphs_carousel.php -->
<div class="card h-full w-full overflow-hidden card-shadow hover-scale">
                ...
                <div class="graphs-container flex-grow relative overflow-hidden">
                    ...
                    <div class="graph-item <?= $index === 0 ? 'active' : '' ?> h-full w-full absolute inset-0 p-3" data-index="<?= $index ?>">
                        <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md h-full w-full flex flex-col">
                            ...
                            <div class="flex-grow flex flex-col p-0 overflow-hidden">
                                ...
                                <div class="flex-grow relative overflow-hidden">
                                    <?php foreach ($groupGraphs as $gIndex => $graph):
                                        ...
                                        $data = $graph['data'];
                                    ?>
                                    <div class="nested-graph-item <?= $gIndex === 0 ? 'active' : '' ?> absolute inset-0 p-3 overflow-hidden" data-group="<?= $index ?>" data-nested="<?= $gIndex ?>">
                                        <div class="h-full w-full flex flex-col overflow-hidden">
                                            <div class="border-b border-gray-200 bg-white px-4 py-2 flex-shrink-0">
                                                <h3 class="text-xs font-medium text-gray-700 text-center truncate">
                                            ...
                                            <div class="flex-grow p-3 overflow-hidden">
                                                ...
                                                <div class="h-full w-full flex flex-col md:flex-row gap-4 overflow-hidden">
                                                                    ...
                                                                    <table class="w-full divide-y divide-gray-200 text-xs">
                                                                        ...
                                                                        <tbody class="bg-white divide-y divide-gray-200">
                                                                            ...
                                                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                                                ...
                                                                                <td class="px-2 py-1 font-medium text-gray-900 text-center truncate" title="<?= htmlspecialchars($item['label']) ?>">
                                                ...
                                                <div class="w-full h-full flex items-center justify-center overflow-hidden">
                                                    <div class="chart-container w-full h-full bar-chart-container">
                                                        <canvas id="nested_graph_<?= $index ?>_<?= $gIndex ?>"></canvas>
                                                    </div>
                                                </div>
                                                <?php endif; ?>
                    ...
                    <div class="graph-item <?= $index === 0 ? 'active' : '' ?> h-full w-full absolute inset-0 p-3" data-index="<?= $index ?>">
                        <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md h-full w-full flex flex-col">
                            ...
                            <div class="flex-grow flex p-0 overflow-hidden">
                                <?php if ($graphType === 'pie'): ?>
                                <!-- For pie charts, show table and chart -->
                                <div class="w-full md:w-1/2 pr-0 md:pr-3 overflow-hidden">
                                    <div class="h-full w-full flex flex-col overflow-hidden">
                                        <div class="overflow-hidden flex-grow mb-3">
                                            <?php 
                                            // Keep original data order for table
                                ...
                                <div class="w-full md:w-1/2 pl-0 md:pl-3 flex items-center justify-center overflow-hidden">
                                    <div class="chart-container w-full h-full">
...
/* Fix for carousel scrolling - DISABLE SCROLLING */
.graphs-carousel {
...
.graphs-carousel::-webkit-scrollbar {
...
.graphs-carousel .flex-grow {
    overflow: hidden;
}

.graphs-carousel .flex-col {
    overflow: hidden;
}
...
/* Prevent scrollbars on all containers */
...
function initSingleGraph(index) {
    ...
    if (graphData[index] && graphData[index].type === 'graph') {
        ...
        console.log(`Successfully initialized individual graph at index ${index}`);
    }
}
...
function initSingleNestedGraph(groupIndex, nestedIndex) {
    ...
    if (graphData[groupIndex] && graphData[groupIndex].type === 'group' && graphData[groupIndex].graphs[nestedIndex]) {
        ...
        console.log(`Successfully initialized nested graph at group ${groupIndex}, nested ${nestedIndex}`);
    }
}
